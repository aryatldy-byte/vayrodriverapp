...
export default function DriverDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // First login (or any account created before this field existed):
      // gate the dashboard behind Terms & Conditions acceptance.
  ...
  const handleAcceptTerms = async () => {
    if (!profile) return;
    try {
      const updated = await updateUserProfile(profile.id, { terms_accepted: true });
      setProfile(updated);
      setShowTerms(false);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-vayroDark">
      <TermsModal open={showTerms} onAccept={handleAcceptTerms} audience="driver" />
      <TopNav
